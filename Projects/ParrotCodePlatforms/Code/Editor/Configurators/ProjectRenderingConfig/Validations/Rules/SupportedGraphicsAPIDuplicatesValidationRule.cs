...
{
    public sealed class SupportedGraphicsAPIDuplicatesValidationRule : ConfigValidationRule<ProjectBuildRenderingConfig>
    {
        public override int Order => 2;

        public override HelpBoxMessage Validate(ProjectBuildRenderingConfig config, [CanBeNull] object data = null)
        {
            var supportedGraphicsAPIDuplicates = config.GraphicsAPI.GroupBy(graphhicsAPI => graphhicsAPI).Where(graphicsAPIGroup => graphicsAPIGroup.Count() > 1).ToArray();
